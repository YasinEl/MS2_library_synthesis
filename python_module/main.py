...
def check_folder_access(paths):
    ...
    for path in folder_paths:
        if path:  # Check if the path is not an empty string
...
if __name__ == "__main__":
    ...
    parser.add_argument("--TautomerLimit", "-tautLim", type=int,
                        ...
                        default=900,
                        required=False)
    parser.add_argument("--OnlySubgroups", "-subgprs", type=bool,
                        help='If the smiles_harmonized column is already present you can skip the harmonization and '
                             'decide to only count subgroups.',
                        default=False,
                        required=False)
    ...
    only_subgrps = args.OnlySubgroups
    is_test = args.test
